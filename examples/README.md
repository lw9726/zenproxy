...
如果管理员关闭“允许新增用户”，那么账号注册、Linux.do 首次登录自动建号、管理员手动建号都会被拒绝；已有用户不受影响。

服务端默认会在每天 `Asia/Shanghai` 时区的 `04:00` 自动刷新订阅；新增/刷新的节点会先测活，通过后立即进入质检。
...
A: 新节点写入后会先进入测活；测活通过后服务端会立刻补一轮质检。空闲时后台还会按 `quality.interval_mins` 做巡检补跑。
